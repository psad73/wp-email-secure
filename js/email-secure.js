function emailSecureDecode(e) {
    if($(e).data('es-decoded')){
        return true;
    }
    var sKey = $('meta[name="x-code-email-sec"]').attr('content');
    var eHref = $(e).attr('href');

    let encryption = new wpEmailSecure();
    var em = eHref.replace(/\.com$/, '').replace(/mailto\:/, '');
    var em1 = encryption.decrypt(sKey, em.substring(0, em.indexOf('@')));
    var em2 = encryption.decrypt(sKey, em.substring(em.indexOf('@') + 1, em.length));
    var decodedEmail = '' + em1 + '@' + em2;
    $(e).attr('href', 'mailto:' + decodedEmail);
    $(e).data('es-decoded', 1);
    return true;
}
...
class wpEmailSecure {
    ...
    decrypt(password, text) {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            const temp = this.getInvertedLetterFromAlphabetForLetter(
                    passwordLetter, letter);
            if (temp) {
                // concat the response
                decrypted += temp;
